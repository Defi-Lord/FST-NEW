...
/** Local scoring: simple, transparent, and self-contained.
 *  - Use player.form when present (FPL-form is a decimal string)
 *  - Fallback to a tiny value from price so non-form players aren’t zero
 *  - Sum across the picked squad
 */
  ...
  return Number(
    team
      .reduce((sum, p) => {
        ...
        const fallback = Math.max(0, (p.price ?? 0) * 0.05) // tiny fallback
          ...
          <div className="list" style={{ marginTop: 12 }}>
            ...
            {table.map((e, i) => {
              ...
              return (
                <div
                  key={`${e.name}-${i}`}
                  className={`card row ${isYou ? 'pill-you' : ''}`}
                >
                  <div style={{ width: 44, textAlign: 'center', fontWeight: 700 }}>
                    {i + 1}
                  </div>
