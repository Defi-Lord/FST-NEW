...
export default function Landing({ onLaunch }: { onLaunch: () => void }) {
  ...
  useEffect(() => {
    ...
    const tryPlay = async () => {
      try {
        ...
        // Some browsers need a user gesture; we'll show the poster until click.
        setCanPlay(false)
      }
    }
  ...
  const handleHeroClick = () => {
    // If autoplay failed, a user tap will start it
    const v = vidRef.current
    if (!v) return
  ...
  return (
    <style>{`
      ...
      .logo {
        ...
        /* background removal tricks:
           - For dark/black backgrounds in the video, "screen" or "lighten" works great.
           - Tune via the CSS variables just below. */
        mix-blend-mode: screen;        /* try 'lighten' if your bg isn't fully dark */
      ...
      /* Subtle edge feather to help blend */
      .logo-mask {
        position:absolute; inset:0;
        -webkit-mask-image: radial-gradient(60% 60% at 50% 50%, #000 60%, rgba(0,0,0,0.0) 100%);
        mask-image: radial-gradient(60% 60% at 50% 50%, #000 60%, rgba(0,0,0,0.0) 100%);
        pointer-events:none;
      }
      ...
      .copy-card {
        ...
        box-shadow: 0 24px 80px rgba(0,0,0,0.35), inset 0 0 80px rgba(255,255,255,0.06);
      }
      .brand {
        font-weight: 900; letter-spacing:.3px; opacity:.92; font-size: clamp(13px, 2.6dvw, 16px);
        margin-bottom: 6px;
      ...
      .cta {
        ...
        border-radius: 14px; padding: 10px 16px; font-weight: 900;
      ...
      .ghost {
        appearance: none; cursor: pointer;
        border-radius: 14px; padding: 10px 16px; font-weight: 800; color:#fff;
      ...
      /* bottom subtle fade */
      .bottom-fade {
        position:absolute; inset:auto 0 0 0; height: 22dvh; z-index: 0;
        background: linear-gradient(180deg, rgba(9,11,16,0.0), rgba(9,11,16,0.85) 40%, rgba(9,11,16,1));
        pointer-events:none;
      }

      @media (max-width: 480px) {
        .logo-shell { width: min(70dvw, 360px); margin-top: 10dvh; margin-bottom: 14px; }
      }
    `}</style>
  )
}
