...
export default function ViewTeam({ onBack, onCreateTeam }: Props) {
      ...
      >
        <div style={{ display:'flex', gap:10, alignItems:'center' }}>
          ...
          <div style={{ flex:1, minWidth:0 }}>
            {/* NAME: now forced to white */}
            <div
              style={{
                fontWeight: 900,
                color: '#ffffff',            // ←–––––––––––––––––––––––––––– name in white
            ...
            {/* SECOND LINE: show CLUB only when selected; otherwise just role chip */}
  ...
  const FormationRow = ({ items }: { items: (any|null)[] }) => {
    ...
    const maxWidth = Math.min(1000, cols * 240)
    return (
